...
import { Home, Library, Upload, Heart, Lock, LogOut } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className="w-64 bg-slate-950/50 backdrop-blur-xl border-r border-white/5 flex flex-col h-full shrink-0 hidden md:flex transition-all">
      <div className="p-6 flex items-center gap-3 mb-6">
        <div className="w-10 h-10 bg-gradient-to-tr from-yellow-400 to-orange-500 rounded-lg flex items-center justify-center shadow-lg shadow-orange-500/20 shrink-0">
          {/* Custom STM Logo - Stylized S and T intertwined */}
          <svg viewBox="0 0 24 24" className="w-6 h-6 text-slate-900" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <path d="M2 10s2-2 4-2 4 2 4 2" className="opacity-0" /> {/* Spacer/Structure */}
            <path d="M16 8c0-2-2-4-5-4s-3 3-3 3" /> {/* Top loop connecting S style */}
          </svg>
        </div>
        <div>
          <h1 className="font-bold text-lg leading-tight tracking-tight">Serhio Tomasito</h1>
      ...
      <div className="p-4 mt-auto border-t border-white/5 space-y-2">
        {isAdmin ? (
          ...
          <button 
            ...
            className="w-full flex items-center gap-2 px-4 py-3 text-slate-500 hover:text-white hover:bg-white/5 rounded-xl transition-colors text-sm"
          >
            <Lock size={16} />
